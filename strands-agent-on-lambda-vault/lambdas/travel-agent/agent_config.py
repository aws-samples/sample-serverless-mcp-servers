...
def get_agent_config(user: User):
    """
    Returns the configuration for the agent.
    """
    l.info(f"Getting agent config for user.id={user.id}")
    
    return {
        "model": __model,
        "system_prompt": __system_prompt,
        "tools": [tools] + mcp_client_manager.get_mcp_tools_for_user(user),
        "session_id": user.id,
        "bucket_name": os.environ.get("STATE_S3_BUCKET_NAME"),
    }
